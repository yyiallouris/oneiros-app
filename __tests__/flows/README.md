...
| `edgeReflectionPrompt.flow.test.ts` | flows-06 (shared client/Edge `oneiros-dream-reflection-v3.2.0`, same-call questions, follow-up chat v2, adaptive depth and routing) |
| `reflectiveQuestions.productionSurfaces.contract.flow.test.ts` | flows-06 **same-call question lock** (Reader v3.2.0 + questions in-reading; Quick 1 / Standard-Advanced 2; chat open 1 / close 0; essays 2; no Composer/Gate/Repair runtime) |
...
| `reflectiveQuestions.productionDeployGuard.contract.flow.test.ts` | flows-06 **fail-closed deploy identity + R&D isolation** (approved same-call `oneiros-same-call-reflective-questions-v1.0.0`; Composer/Gate/Repair/Premise denied; recovered v105 retained; failed Inviter/editorial SHAs denied) |
