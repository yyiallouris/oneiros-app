...
| `reflectiveQuestions.productionDeployGuard.contract.flow.test.ts` | flows-06 **locked deploy hold + R&D isolation** (runtime prompt `src/ai/reflectiveQuestionPrompt.ts` SHA `4885e351…`; fail-closed `npm run deploy:ai-entitlements-gateway`; R&D under `src/ai/rd/reflective-questions/` is not a runtime import; Candidate B SHA `08cd3eaf…` frozen; mega-runner archived) |
