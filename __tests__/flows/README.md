...
| `edgeReflectionPrompt.flow.test.ts` | flows-06 (shared client/Edge `oneiros-dream-reflection-v3.1.0-candidate`, frozen Reader, production question orchestration, prose-only Dialogue, adaptive depth and routing) |
| `reflectiveQuestions.productionSurfaces.contract.flow.test.ts` | flows-06 **Reflective Dialogue / Questions product lock** (frozen Reader + production orchestration always-one schema-11; localized fallback; Dialogue `1.9.1` + optional v5 chat question; final none; post-typing card; essays unchanged; same-call v1.2 frozen Generator; Integrity Gate + Premise Check + one Repair) |
...
| `reflectiveQuestions.productionDeployGuard.contract.flow.test.ts` | flows-06 **fail-closed deploy identity + R&D isolation** (approved orchestration `oneiros-reflective-question-production-v1.0.0`; chat v5 remains separate; standalone Generator/Gate/Repair/Premise denied; recovered v105 retained; failed Inviter/editorial SHAs denied) |
