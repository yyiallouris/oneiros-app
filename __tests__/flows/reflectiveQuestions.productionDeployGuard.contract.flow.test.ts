...
describe('reflective question production deploy guard contract', () => {
  ...
  it('keeps the production prompt as the runtime source of truth and R&D isolated', () => {
    ...
    const prompt = read('src/ai/reflectiveQuestionPrompt.ts');
    expect(prompt).toMatch(/reflective-question-psychological-aliveness-v1\.4\.0/);
    expect(read('src/services/ai.ts')).toMatch(
