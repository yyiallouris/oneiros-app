...
- **Depth:** interpretation depth preference with **Core Reflection (recommended)** preselected. The three levels use the same concise, reader-facing descriptions in onboarding and Account: a short reflection on one clear thread, a fuller reading of the dream’s images and inner movement, or an extended reflection when the reader wants to explore the dream further. Continue persists the selection. No Skip — with a default already chosen, Skip would be a false choice.
...
- **Dream analysis — level:** `quick` | `standard` | `advanced` (stored via `userSettingsService` on select), using the same shared reader-facing labels and descriptions as onboarding.
