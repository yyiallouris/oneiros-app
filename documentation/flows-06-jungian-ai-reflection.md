...
**Status: locked engineering contract. Launch topology is same-call. Canonical release `v1.0.3` / alias `oneiros-same-call-reflective-questions-v1.0.3` maps to immutable evaluated artifact `oneiros-same-call-reflective-questions-v1.0.3-candidate` / SHA `f5399a49…`; Reader alias `oneiros-dream-reflection-v3.2.3` maps to `oneiros-dream-reflection-v3.2.3-candidate`. Gateway function version `113` is live with that artifact, normalizer `oneiros-reflective-question-structure-normalizer-v1.0.0`, and runtime bundle `oneiros-reflective-questions-runtime-v1.0.3+structure-v1.0.0`. Its one frozen 21-call evaluation passed Q1 `21/21`; the separately versioned normalizer closes the one Chinese missing-heading failure without changing model-owned prose/questions or partial streaming. Q2 is unchanged and remains a separate editorial-quality task. Archived `v1.0.2-candidate` / SHA `94d4a92a…` remains denied. There is no current production blocker.** One Reader/chat/essay call writes the production reading and questions. Composer, v1.2 orchestration, Integrity Gate, Repair, Premise Check, and question fallback are **CLOSED R&D — not production**. Deployment record: [`../docs/REFLECTIVE_QUESTION_PRODUCTION_HOLD.md`](../docs/REFLECTIVE_QUESTION_PRODUCTION_HOLD.md). v1.0.3 evaluation review: [`../docs/ONEIROS_V103_ENACTED_RELATION_EVALUATION_REVIEW_2026-08-29.md`](../docs/ONEIROS_V103_ENACTED_RELATION_EVALUATION_REVIEW_2026-08-29.md).
...
- **Symbolic reflection:** preview of the assistant reflection, using the first meaningful prose excerpt rather than stopping on a bare heading like `Core Tension`; this preview sits below **Explore symbolic layers** and opens with its own local section label so the prose does not begin abruptly.
