...
**Status: locked engineering contract. Launch production is same-call: `oneiros-same-call-reflective-questions-v1.0.0` / SHA `25b1114a…`.** One Reader/chat/essay call writes the reading and the questions. Composer, v1.2 orchestration, Integrity Gate, Repair, Premise Check, and question fallback are **CLOSED R&D — not production**.
...
**Multilingual:** production keeps the shared 12-language source contract. Question text uses the requested/dominant dream language. No runtime translation pass. Architecture: [`../docs/REFLECTIVE_QUESTIONS_V2_ARCHITECTURE.md`](../docs/REFLECTIVE_QUESTIONS_V2_ARCHITECTURE.md), R&D FAIL record [`../docs/ONEIROS_QUESTION_REPAIR_PHASE2_EDITORIAL_FAIL_2026-08-29.md`](../docs/ONEIROS_QUESTION_REPAIR_PHASE2_EDITORIAL_FAIL_2026-08-29.md).

Questions close the reading naturally. Continue Exploring remains available. Chat continues the conversation; it is not a second interpretation engine.
...
- Reading prompt: `oneiros-dream-reflection-v3.2.0`. Method: `oneiros-same-call-reflective-questions-v1.0.0` / SHA `25b1114a…`. Follow-up chat: `oneiros-followup-chat-v2.0.0`.
...
- Pipeline: one `interpretation_*` call writes reading + questions. Missing/extra cardinality uses the existing whole-response retry, never a second question call. Deterministic markdown extraction may store `reflectiveQuestions`.
- Continue Exploring stays available. Questions appear in the typed reading, not as a high-salience card.
- Essays: `2.0.3-phase1`, exactly 2 same-call questions.
...
7. One GPT-5.4 Reader call writes a complete reading that already includes the required reflective question(s). The gateway streams visible reading bytes, including those questions. Historical `BEGIN_DREAM_READING` envelopes remain salvageable. There is no second question inference.
8. When the background task saves the canonical interpretation with `metadata_status: pending` and commits quota, the status poll returns the interpretation payload and the client replaces the temporary partial message with the persisted interpretation without a second immediate remote fetch. Questions remain in the typed reading. Continue Exploring is available without answering.
