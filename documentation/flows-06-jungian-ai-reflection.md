...
**Status: locked engineering contract. Production orchestration is approved: `oneiros-reflective-question-production-v1.0.0` / SHA `fc8b6304…`.** Product invariant: every successful initial Oneiros reading always has exactly one reflective question. Frozen Reader (`oneiros-dream-reflection-v3.1.0-candidate`) plus Generator v1.2 → Integrity Gate → Premise Check → at most one Repair → Gate → Premise Check → deterministic localized fallback. Schema `11`. Standalone Generator/Gate/Repair/Premise Check method IDs remain denied for deploy-as-the-production-method.

**Closed Inviter R&D, not part of this flow:** Post-Jungian Inviter v2.0.1 (`09045bf1…`) failed locked blind review at `0 CLEAR PASS / 1 BORDERLINE / 7 FAIL` against frozen corpus `2a1a8bc3…` (`$0.043675`). Post-reading Inviter v1 (`70c533e5…`) failed at `1 CLEAR PASS / 1 BORDERLINE / 6 FAIL`. Editorial-arc v2 (`6cd304e1…`) failed at internal `2 CLEAR PASS / 6 FAIL`. All remain denied. No `v2.0.2`, novelty discriminator, Director, Composer-as-judge, or `no_question` chase.

**Multilingual:** production keeps the shared 12-language source contract and localized fallback `dream_reflective_question_fallback` (`reflective-question-fallback-v1`). Mechanical validation does not require ASCII `?`. Architecture: [`../docs/REFLECTIVE_QUESTIONS_V2_ARCHITECTURE.md`](../docs/REFLECTIVE_QUESTIONS_V2_ARCHITECTURE.md), R&D FAIL + launch addendum [`../docs/ONEIROS_QUESTION_REPAIR_PHASE2_EDITORIAL_FAIL_2026-08-29.md`](../docs/ONEIROS_QUESTION_REPAIR_PHASE2_EDITORIAL_FAIL_2026-08-29.md).

The question is the first bridge into Exploring. Continue Exploring remains available without answering. Chat may deepen with zero or one question; final chat asks none; the typed card and streamed reveal remain unchanged.

Use `npm run deploy:ai-entitlements-gateway` only. The wrapper hashes the production orchestration bundle and fails closed unless it matches the approved identity. An environment token cannot approve a denied SHA. Client and gateway request paths must not import the hold module. Frozen Generator/Gate/Repair reach gateway only through `src/ai/reflectiveQuestionPipeline.ts`. Kill switch `ONEIROS_REFLECTIVE_QUESTION_KILL_SWITCH` (default off) is the emergency omit path; otherwise never silence.
...
**Status: locked product contract (updated 2026-08-29).** Oneiros always leaves the dreamer with a question. The question does not have to manufacture a problem.

- Reading prompt: `oneiros-dream-reflection-v3.1.0-candidate` remains frozen. The production question path is orchestration `oneiros-reflective-question-production-v1.0.0` / SHA `fc8b6304…`, not Composer.
- Approved local question method: `oneiros-reflective-question-production-v1.0.0`. Premise Check `oneiros-question-premise-check-v1.0.0-candidate` / SHA `ceca4568…`. Structured Composer v1.0.1 / SHA `56150c82…` remains readable historical evidence. Post-Jungian Inviter v2.0.1 / SHA `09045bf1…` is denied after mechanical `8/8` but blind human `0 PASS / 1 BORDERLINE / 7 FAIL`. Editorial arc / SHA `6cd304e1…` remains historical failed evidence.
- Initial Quick / Standard / Advanced: always exactly one display question. Depth maps `quick→core`, `standard→core`, `advanced→deeper`. Core is never dumb. Deeper holds more psychic complexity only when earned. There is no separate Standard question mode.
- The question is one natural post-Jungian opening. There is no invitation taxonomy and no `kind` on new writes.
- Chat non-final: zero or one after the actual answer. Chat final: no question and no question-model calls.
- Initial pipeline: complete numbered D# → frozen Reader + v1.2 Generator (same call) → Integrity Gate → Premise Check → at most one Repair → Gate → Premise Check → schema-11 `status: "question"` (`source: generator | repair | fallback`). Provider/schema failure uses the localized fallback. No second Repair, no rejected-question leak, no editorial judge.
- Chat pipeline: restore visible question → Dialogue answer → D# plus user-only U# → optional single-pass question call. Final chat skips it; assistant prose never becomes evidence.
- If the user answers the initial question, Exploring receives the raw dream, final reading, exact question, and the user's answer. Continue Exploring stays available without answering.
- An initial question-path failure never fails or removes a successful reading; it still shows the fallback question unless the kill switch is on (emergency omit).
- Essays: `2.0.3-phase1`, exactly one. Do not inject the method. Do not revert to remote `2.0.0` 1–2.
- Deploy only through `npm run deploy:ai-entitlements-gateway`. The wrapper hashes the orchestration bundle against the approved identity. Closed experiments stay out of selectable production routing.
- Contract tests: `__tests__/reflectiveQuestionPipeline.test.ts`, `__tests__/questionPremiseCheck.test.ts`, `__tests__/ReflectiveQuestionCard.test.tsx`, `__tests__/flows/reflectiveQuestions.productionSurfaces.contract.flow.test.ts`. Records: `docs/REFLECTIVE_QUESTIONS_V2_ARCHITECTURE.md`, `docs/REFLECTIVE_QUESTION_PRODUCTION_HOLD.md`, `docs/ONEIROS_QUESTION_REPAIR_PHASE2_EDITORIAL_FAIL_2026-08-29.md`.
- Same-call v1.2.0 (`oneiros-same-call-minimal-v1.2.0-candidate`, SHA `4506c898…`) is the frozen Generator. Prompt R&D on System 4 is STOP. Integrity Gate and Repair prompts stay frozen. HOME Standard (`ja-neon-home:standard`) must not reach final output unchanged.
...
7. The same GPT-5.4 same-call writes a complete reading and one candidate question. The gateway reveals only the reading segment (`visibleSameCallReading`) and then runs Integrity Gate + Premise Check (at most one Repair) before save. Schema-11 artifacts are `question` from generator, repair, or localized fallback. Historical `BEGIN_DREAM_READING` envelopes remain salvageable.
8. When the background task saves the canonical interpretation with `metadata_status: pending` and commits quota, the status poll returns the interpretation payload and the client replaces the temporary partial message with the persisted interpretation without a second immediate remote fetch. The question card appears only after `PhasedTypingText` has settled. The UI does not label generator / repair / fallback.
...
- Client and gateway import the same `oneiros-dream-reflection-v3.1.0-candidate` initial builder. Body stays in the dream's primary language; headings stay English. The optional opening remains outside reading prose and UI streaming.
- Reflective Questions owns presentation cardinality: initial zero or one; chat zero or one; final chat none. Initial uses raw-dream D# only. Chat uses D# plus user-authored U# and treats assistant prose as provisional orientation.
...
Advanced mode means “linger longer, not explain more.” Length is earned by transformations, contradictions, and psychic resolution; `250–400` and `650–800` are telemetry bands, never minimum/maximum quality constraints. The optional question is never placed in prose.
...
- **Optional conversation-opening question:** after the preview, a quiet localized card shows a validated question. A valid `no_question` renders no card and keeps only neutral localized “Continue from here”; generic `Continue exploring` remains a legacy/rejection fallback.
