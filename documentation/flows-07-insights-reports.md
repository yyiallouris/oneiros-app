...
- Recent Dream Field and Period Reflection keep the shared topology-first prompt from `src/ai/reflectiveEssayPrompt.ts`: `oneiros-recent-dream-field-v2` and `oneiros-period-reflection-v2`, now `2.0.4-phase1`. This is a narrow launch-contract patch over the accepted `2.0.3-phase1` baseline: metadata-heavy context version `1`, model routing, temperatures, sections, word policy, and the single operational whole-essay retry remain unchanged. Before interpretation, the prompt privately chooses one supported field, parallel/local clusters, or a loose field and must preserve that topology through every section and exactly two same-call reflective questions. A shared stance requires comparable situation → comparable affective stance → comparable response; abstract equivalence and an opening disclaimer cannot substitute for recurrence. Weekly scopes receive week headings; monthly scopes receive month headings; Recent remains a lighter current sequence rather than a miniature monthly essay. Structural question validation is fail-open shadow telemetry only: neither a contract failure nor an observer exception triggers a rewrite. The existing retry is limited to incomplete or initially over-limit essays. Headings stay in English; body and questions use the requested language; hidden essay completion markers are stripped before persistence.
...
- Phase 1 is accepted and frozen. The PO treats weak atmospheric affinity in a loose field as a documented ambiguity rather than a blocker: “no sufficiently dense unified field” does not forbid every cross-dream similarity, provided it is not promoted into a dense symbolic or developmental thesis. Phase 2 narrative-first scored `7 PASS / 2 FAIL`; its one permitted Field Map architecture spike then scored manual `2 PASS / 7 FAIL`, failed the parallel-cluster target, and degraded coherent-field sensitivity. The stop rule closes Phase 2 R&D. Phase 1 `2.0.3-phase1` plus context version `1` remains the accepted intelligence baseline; production `2.0.4-phase1` changes only same-call question cardinality/validation. See `docs/ONEIROS_REFLECTIVE_ESSAYS_FIELD_MAP_SPIKE_REVIEW_2026-08-26.md`.
