...
5. The Reader (`oneiros-dream-reflection-v3.2.0`) writes a complete reading that already includes the required reflective question(s). Streaming/typewriter reveals that same markdown, including questions.
6. There is no second question inference. Quick ends with one terminal question. Standard/Advanced end with exactly two bullets under `## Reflective Questions`. Deterministic extraction may store `reflectiveQuestions`. Continue Exploring stays available without answering.
7. On follow-up, `oneiros-followup-chat-v2.0.0` continues the existing conversation. Non-final replies end with one natural question; closing replies ask none. Chat is not Gate/Repair/Composer.
...
10. DreamDetail renders:
   ...
   - Assistant reflection, with same-call questions in the reading markdown. Continue Exploring is available without answering (nested Exploring `ScrollView` rules remain locked).
...
Approved identity: `oneiros-same-call-reflective-questions-v1.0.0` / SHA `25b1114a…`. Reader `oneiros-dream-reflection-v3.2.0`. Chat `oneiros-followup-chat-v2.0.0`. Composer, Gate, Repair, Premise Check, and v1.2 orchestration remain closed R&D. Record: [`../docs/REFLECTIVE_QUESTION_PRODUCTION_HOLD.md`](../docs/REFLECTIVE_QUESTION_PRODUCTION_HOLD.md).
...
- **Reflective-question production:** approved same-call `oneiros-same-call-reflective-questions-v1.0.0` / SHA `25b1114a…`. Reader `oneiros-dream-reflection-v3.2.0`. Chat `oneiros-followup-chat-v2.0.0`. Quick 1 / Standard-Advanced 2 / chat non-final 1 / closing 0 / essays 2. No second question LLM. Exploring `chat_followup` stays `gpt-5.4-mini`. Locked streaming feel, extraction, Echoes, `archetypes`, and `amplifications` are unchanged. Deploy only through the guarded wrapper. Closed Inviter/editorial/orchestration SHAs remain denied.
