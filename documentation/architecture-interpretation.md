...
- That shared prompt includes a SOURCE BOUNDARY: Dream Fabric fields must be grounded in dream text only; Interpretive Echoes stay provisional. Archetypes use selection theory (decisive turning-point coverage before ranking + archetypal weight + candidate comparison) plus an operational catalog with `kind` / `selectWhen` / `insufficientWhen` / `displayLabel` (no production `evaluation` bag; diagnostics only in `__DEV__`). Mythic Echo is open-world 0–1 (`prompt_id` `dream-field-map-interpretive-v3.6`, `prompt_version` `3.6.7`, schema `4`) with configuration-first reasoning, narrative-specificity hierarchy (specific tale > cycle > generic complex > motif), winner consistency on sequence/roles/defining-action/turning-point, and a lightweight title/evidence/confidence validator. No mythology corpus/resolver. Dream Detail Fabric / Inner Tensions UI is unchanged; empty echo subsections are hidden; archetypal titles use catalog `displayLabel`; mythic shows title + muted tradition separately.
