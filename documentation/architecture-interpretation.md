...
- That shared prompt includes a SOURCE BOUNDARY: Dream Fabric fields must be grounded in dream text only; Interpretive Echoes stay provisional. Archetypes use a closed whitelist + concise hard gates for Double / Guide / Divine Child / Terrible Mother / Ruler (no production `evaluation` bag; candidate notes only in `__DEV__` interpretive_diagnostics). Mythic Echo is open-world 0–1 output of the same call (`prompt_id` `dream-field-map-interpretive-v3.6`, `prompt_version` `3.6.3`, schema `4`), followed by a lightweight title/evidence/confidence validator. Titles must name a narrative/episode (not a bare figure); localized title when available, else scholarly. No mythology corpus/resolver. Precision over coverage — `[]` preferred over a false echo, but do not omit highly distinctive multi-stage structural matches (normally return when ≥4 sequenced correspondences + defining action/prohibition + qualifying divergence). Dream Detail Fabric / Inner Tensions UI is unchanged; only Archetypal/Mythic generation copy is shortened (mythic renders as one compact paragraph).
