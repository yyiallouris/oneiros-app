...
Canonical release `v1.0.3` / alias `oneiros-same-call-reflective-questions-v1.0.3` maps to immutable approved artifact `oneiros-same-call-reflective-questions-v1.0.3-candidate` / SHA `f5399a49…`; Reader alias `oneiros-dream-reflection-v3.2.3` maps to artifact `oneiros-dream-reflection-v3.2.3-candidate`. The production package also carries Chat `oneiros-followup-chat-v2.0.1`, normalizer `oneiros-reflective-question-structure-normalizer-v1.0.0`, and runtime bundle `oneiros-reflective-questions-runtime-v1.0.3+structure-v1.0.0`. Aliases are release metadata only; prompt bytes and runtime artifact ids stay frozen. The shared 12-language resolver runs before generation; deterministic marker/language/cardinality/no-answer-menu checks run after completion as no-retry shadow telemetry. `safeObserveReflectiveContract` catches observer exceptions, records `passed: null` plus a compact error diagnostic, and lets the successful generation continue unchanged. Composer, Gate, Repair, Premise Check, and v1.2 orchestration remain closed R&D. Production Q2 is unchanged. Q2-only v1.0.4 imaginal-handoff (`a4f972c…`) and final v1.0.5 source-ownership (`16da1d13…`) completed one frozen evaluation each and are HOLD/denied; v1.0.5 failed control, unseen, Q1-regression, and recurring-family gates, so Q2 prompt R&D is stopped. Neither has a runtime import or deployment. Records: [`../docs/REFLECTIVE_QUESTION_PRODUCTION_HOLD.md`](../docs/REFLECTIVE_QUESTION_PRODUCTION_HOLD.md), [`../docs/ONEIROS_V104_IMAGINAL_HANDOFF_EVALUATION_REVIEW_2026-08-29.md`](../docs/ONEIROS_V104_IMAGINAL_HANDOFF_EVALUATION_REVIEW_2026-08-29.md), [`../docs/ONEIROS_V105_SOURCE_OWNERSHIP_EVALUATION_REVIEW_2026-08-29.md`](../docs/ONEIROS_V105_SOURCE_OWNERSHIP_EVALUATION_REVIEW_2026-08-29.md).
