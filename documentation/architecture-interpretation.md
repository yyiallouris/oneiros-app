...
- The gateway initial reflection prompt must stay in parity with the canonical client `src/services/ai.ts` initial interpretation contract: same constitution / role / recovered production reflective-question method / format / user-message shape, same-language body text and questions, and the same surface cardinality (Quick: exactly one through the method; Standard / Advanced: 1–2 default 1). Essays stay on `2.0.3-phase1` exactly one and do not receive the method as an extra system message.
...
- **Reflective-question production:** recovered remote `ai-entitlements-gateway` v105 is `reflective-question-psychological-aliveness-v1.4.0` SHA `4885e351…` (docs had said “v1.5 bundle”). Canonical runtime prompt is `src/ai/reflectiveQuestionPrompt.ts` (client + gateway). Essays stay `2.0.3-phase1` exactly one and do not inject the method. Do not deploy local Oneiros Reader v1.4.0. Use `npm run deploy:ai-entitlements-gateway`. R&D is isolated under `src/ai/rd/reflective-questions/` with Candidate B as the only active research base. The hold module and R&D modules are not imported by client or gateway runtime. Record: `docs/REFLECTIVE_QUESTION_PRODUCTION_HOLD.md`.
