...
- **Reflective-question production hold:** recovered remote `ai-entitlements-gateway` v105 is `reflective-question-psychological-aliveness-v1.4.0` SHA `4885e351…` (docs had said “v1.5 bundle”). Do not deploy local Oneiros Reader v1.4.0. Use `npm run deploy:ai-entitlements-gateway`. R&D is isolated under `src/ai/rd/reflective-questions/` with Candidate B as the only active research base. Record: `docs/REFLECTIVE_QUESTION_PRODUCTION_HOLD.md`. The recovered snapshot and R&D modules are not imported by client or gateway runtime.
