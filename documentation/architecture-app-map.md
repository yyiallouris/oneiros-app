...
- Reflective essay input ownership: `src/ai/reflectiveEssayContext.ts` builds accepted metadata-heavy production context version `1`; its narrative-first version `2` builder remains research-only. `src/ai/reflectiveEssayPrompt.ts` owns `2.0.4-phase1`: the accepted `2.0.3` topology, sections, temperatures, operational incomplete/length retry policy, plus the two-question same-call prompt contract. Structural question results are observed only after completion. Client and gateway select the metadata-first builder; `src/ai/reflectiveEssayFieldMapSpike.ts` is used only by the rejected offline architecture harness.
