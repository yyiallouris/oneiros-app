...
2. **Auto-save draft:** after ~2s idle when `content` non-empty → `saveDraft` (today’s date).
   - Write and Dream Editor use the exact dream-body prompt **“Write it as you remember it.”** The guidance invites recall without adding an instruction about correcting or editing.
...
- Dream entries render as lightweight archive slips, not heavy cards. Each slip shows a small date seal, then copy: an explicit user-authored title uses the shared serif dream-title role and stays a strong one-line heading plus a two-line excerpt. An untitled dream uses a restrained regular-weight lead from the opening words in the shared sans role, ending with `...`, then a two-line excerpt that continues those words on a word boundary with no mid-word cut; it never implies that Oneiros or the user assigned a stored title. Tiny symbolic markers (Image, Place, Atmosphere when available) follow as separate low-contrast capsules using natural `Label: value` syntax, so adjacent metadata never runs together as slash-delimited text.
- Tap dream → **`DreamDetail`**.
- Header calendar icon → **`Calendar`**.
- The Journal header preserves its editorial spacing while clearing the device top safe area. Its list uses the shared floating-tab content inset, including the bottom safe area, so the final slip can scroll fully above the parchment shelf. The shelf hides while the search keyboard is open. The navigation family keeps distinct concepts in one authored hand: Write preserves the authored open dry-brush feather in a compact `30 × 28dp` crop with a restrained low-opacity pressure copy, keeping the visible mark in the shared navigation band; Journal keeps the open book with a perceptible page lift, stronger unequal outer pressure, a softly wandering spine, and faint overlapping edge traces; Insights uses the authored hand-ink eye through a `29dp` crop that excludes the two detached witness dots. The eye remains the fully open navigation seeing mark; Images inside Insights uses a separate half-lidded raster with a vertical organic presence and one witness dot. Active tabs use Night Plum at `0.98` with medium labels; inactive glyphs recede to `0.58` in Muted Tab Ink while labels retain accessible contrast. Neither state adds a pill, badge, or separate selection dot. Calendar and microphone share a 31dp optical frame and Muted Ink colour, while preserving different media inside one pressure-led functional dialect. Microphone keeps its narrow native-SVG capsule, unequal open cradle, wandering stem, short weighted base, and faint inner breath trace. Calendar uses the final Oneiros v1 artifact `oneiros-calendar-date-leaf-v1.0.0`, a transparent authored raster made from irregular filled ink masses: broken perimeters, dry gaps, low-alpha filaments, and a softly lifted lower corner give it actual stroke texture rather than simulated SVG pressure. The page lift relates it quietly to Journal without copying the open-book silhouette; two unequal bindings, one broken header gesture, one main date trace, and one fading memory trace keep it legible without a rigid planner grid. Neither uses moon/star/sparkle or sound-wave shorthand. Search/edit/send/copy/chevrons retain their paths and use the quieter rounded functional stroke. Every parent control keeps at least a 44dp touch target.
