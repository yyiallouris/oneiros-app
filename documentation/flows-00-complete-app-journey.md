...
- `DreamDetailScreen`: shows the dream, streamed reflection, `display_distillation`, and exactly one reflective question after the reading. The frozen Reader writes the reading inside the same-call Generator; production orchestration then returns one question (generator, repaired, or localized fallback). Continue Exploring remains available without answering. Dialogue `1.9.1` restores an exact visible question when present; later v5 questions remain optional.
