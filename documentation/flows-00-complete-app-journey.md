...
- Bottom navigation now renders as a floating parchment shelf over the paper background with explicit active/inactive PNG icon pairs. Tab screens share `floatingTabBar` tokens so primary CTAs (Write **Save dream**) sit in the layout above the shelf instead of slipping underneath it.
