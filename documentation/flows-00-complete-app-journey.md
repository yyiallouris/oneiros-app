...
- Bottom navigation renders as a translucent parchment shelf (`86%` opaque / `14%` transparent) over the paper background. Feather and sun retain their existing active/inactive PNG artwork; Journal alone uses the approved lighter open-journal drawing with one subtly upward-drifting page edge, unequal lower-curve cadence, variable ink pressure, and a softly wandering spine. Its dream quality comes from that restrained page movement, not from an added symbol, dot, or badge; focus is expressed through ink strength and opacity. Shared `iconography` tokens normalize state opacity and optical size without recolouring Insights or using colour as proof of icon consistency. Tab screens share `floatingTabBar` tokens so primary CTAs (Write **Save dream**) sit in the layout above the shelf instead of slipping underneath it.
