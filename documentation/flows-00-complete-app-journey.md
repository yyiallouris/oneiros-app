...
- `DreamDetailScreen`: shows the dream, streamed reflection, `display_distillation`, and same-call reflective questions inside the reading (Quick: 1 terminal question; Standard/Advanced: 2 under Reflective Questions). Continue Exploring remains available. Follow-up chat `oneiros-followup-chat-v2.0.0` continues the conversation with one trailing question on open turns and none when closing.
