...
| AI interpretation and chat | `DreamDetailScreen`, `InterpretationChatScreen`, `dreamReflectionPrompt.ts`, `reflectiveQuestionPipeline.ts`, `questionPremiseCheck.ts`, `reflectiveEvidence.ts`, `reflectiveDialogueResponseFormat.ts`, `reflectiveQuestionPrompt.ts`, `reflectiveLanguage.ts`, `ai.ts` | Frozen Reader then always-one production question (schema 11: generator, one Repair, or localized fallback); gateway streams only reading and persists the question card. Dialogue `1.9.1` keeps visible-question replay and optional v5 D#/U# follow-ups. Generator/Gate/Repair prompts stay frozen; Premise Check is the narrow final evidence check. | flow 06 (always-one question, frozen Reader, multilingual fallback, metadata resilience, locked streaming, fail-closed rollout), architecture interpretation, protocol/display/trajectory tests |
