...
| AI interpretation and chat | `DreamDetailScreen`, `InterpretationChatScreen`, `dreamReflectionPrompt.ts`, `reflectiveQuestionExtract.ts`, `reflectiveEssayPrompt.ts`, `reflectiveLanguage.ts`, `ai.ts` | Same-call Reader writes reading + questions (Quick 1, Standard/Advanced 2). Follow-up chat continues the conversation (open 1 question, closing 0). Composer/Gate/Repair remain closed R&D. | flow 06 (same-call questions, follow-up chat, metadata resilience, locked streaming, fail-closed rollout), architecture interpretation, extract/prompt contract tests |
