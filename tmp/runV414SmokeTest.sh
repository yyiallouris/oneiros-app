...
if not m: raise SystemExit("no service_role")
print(m.group(1))
')
EMAIL="oneiros.v414.smoke+$(date +%s)@example.com"
PASS="V414Smoke!$(openssl rand -hex 4)"
...
trap 'curl -sS -X DELETE "'"$URL"'/auth/v1/admin/users/'"$USER_ID"'" -H "apikey: '"$SERVICE"'" -H "Authorization: Bearer '"$SERVICE"'" >/dev/null || true' EXIT
export LIVE_SUPABASE_EMAIL="$EMAIL" LIVE_SUPABASE_PASSWORD="$PASS"
export EXPO_PUBLIC_SUPABASE_URL="$URL" EXPO_PUBLIC_SUPABASE_ANON_KEY="$ANON"
export EXPO_PUBLIC_CUSTOM_GPT_ENDPOINT="$ENDPOINT"
npx --yes tsx tmp/runV414SmokeTest.ts
