#!/usr/bin/env bash
# Patch B.2 benchmark: Fisherman ×7 + figure/action positives + 5 negatives. Fully parallel.
set -euo pipefail
cd "$(dirname "$0")/.."
...
EMAIL="oneiros.v413b2+$(date +%s)@example.com"
PASS="V413B2!$(openssl rand -hex 4)"
...
npx --yes tsx tmp/runPatchB2TricksterBenchmark.ts
