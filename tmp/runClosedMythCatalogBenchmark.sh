...
EMAIL="oneiros.v41.closed+$(date +%s)@example.com"
PASS="V41ClosedTest!$(openssl rand -hex 4)"
...
# Ensure proxy endpoint is set (same as app .env if present)
if [ -z "${EXPO_PUBLIC_CUSTOM_GPT_ENDPOINT:-}" ]; then
  export EXPO_PUBLIC_CUSTOM_GPT_ENDPOINT=$(python3 - <<'PY'
from pathlib import Path
import re
raw=Path('.env').read_text()
...
npx --yes tsx tmp/runClosedMythCatalogBenchmark.ts
