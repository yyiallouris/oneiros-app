#!/usr/bin/env bash
# Patch B benchmark: Fisherman ×7 + 5 negative Trickster cases. Fully parallel by default.
set -euo pipefail
cd "$(dirname "$0")/.."
...
EMAIL="oneiros.v413.patchb+$(date +%s)@example.com"
PASS="V413PatchB!$(openssl rand -hex 4)"
...
export ACCEPTANCE_CONCURRENCY="${ACCEPTANCE_CONCURRENCY:-12}"

npx --yes tsx tmp/runPatchBFishermanTrickster.ts
