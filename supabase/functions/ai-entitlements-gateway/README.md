...
- that contract includes a SOURCE BOUNDARY (Dream Fabric vs Interpretive Echoes), compact Dream Motif `motifs` (Insights later call aggregated motifs Recurring Scenes), felt-tone-only `affects`, pattern-label `relational_dynamics`, short `thresholds`, converging-evidence Archetypal Echoes in `archetypes` (0–2 objects `{ canonical_label, expression, resonance, evidence }`; classical whitelist first), and rare Mythic Echoes in `amplifications` (0–1 named parallel `{ title, tradition, resonance, difference, evidence }` on structural correspondence, not Fabric / not Forming Patterns)
