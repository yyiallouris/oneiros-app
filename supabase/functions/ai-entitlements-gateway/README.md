...
- that contract includes a SOURCE BOUNDARY (Dream Fabric vs Interpretive Echoes), compact Dream Motif `motifs`, felt-tone-only `affects`, pattern-label `relational_dynamics`, short `thresholds`, closed-whitelist Archetypal Echoes in `archetypes` (0–2; no production `evaluation` bag), and open-world Mythic Echo (0–1) as a direct `amplifications` output of the same extraction call (narrative/episode titles only, not bare figures), with a lightweight post-validator. Current ids: `dream-field-map-interpretive-v3.6` / `prompt_version` `3.6.7` / schema `4`. Decisive turning-point coverage + operational archetype catalog; mythic specificity hierarchy + winner consistency.
- optional `debug_interpretive_echoes: true` on `dream_metadata_extract` **forces a fresh extraction** (bypasses ready-cache) and returns `debug_interpretive_echoes` with `interpretive_diagnostics`, `post_validation_archetypes`, `post_validation_amplifications`, plus prompt/model metadata; diagnostics are never written into the interpretation row and must never render in Dream Detail UI
