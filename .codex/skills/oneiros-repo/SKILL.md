...
- `archetypes` (v3.6.7): identify decisive turning-point / power-reversal action first, then carrier coverage across figures/actions/relationships/configurations/transformations; operational catalog; no production `evaluation` bag; max 0–2; Ego not user-facing on Dream Detail.
- `amplifications` Mythic Echo: configuration-first; merge aliases of the same specific tale but keep specific tale > cycle > generic complex > motif; winner = strongest sequence/roles/defining-action/turning-point (not broad restoration frame); lightweight post-validator. UI: title + muted tradition; subsection `Mythic Echo`.
- Extraction prompt id `dream-field-map-interpretive-v3.6` / `prompt_version` `3.6.7` / schema `4`. Debug diagnostics include `decisive_turning_point`, `carrier_kind`, mythic dimensional scores / `aliases_merged` / `narrative_specificity` / `gate_failure`; force-debug bypasses ready-cache.
