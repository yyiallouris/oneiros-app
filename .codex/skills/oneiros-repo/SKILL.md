...
- **Reflective dialogue/question production:** every successful reading ends with exactly one question. Approved orchestration is `oneiros-reflective-question-production-v1.0.0` / SHA `fc8b6304…`. Frozen Reader `oneiros-dream-reflection-v3.1.0-candidate` plus Generator v1.2 (`4506c898…`) → Integrity Gate (`c1d8090f…`) → Premise Check (`ceca4568…`) → at most one Repair (`0859fd54…`) → Gate → Premise Check → deterministic localized fallback. Schema `11`; schemas 1–10 remain readable. Depth maps `quick→core`, `standard→core`, `advanced→deeper`. Kill switch `ONEIROS_REFLECTIVE_QUESTION_KILL_SWITCH` (default off) is the emergency omit path. Chat keeps Dialogue `1.9.1` plus v5 optional follow-ups; final none. Exploring `chat_followup` stays `gpt-5.4-mini`. Do not mutate frozen prompts. Do not reopen question-grammar R&D. Closed Inviter/editorial SHAs stay denied. Gateway may import frozen Generator/Gate/Repair only through `src/ai/reflectiveQuestionPipeline.ts`. Deploy only through `npm run deploy:ai-entitlements-gateway`. Canonical FAIL + launch addendum: `docs/ONEIROS_QUESTION_REPAIR_PHASE2_EDITORIAL_FAIL_2026-08-29.md`. Guards: production surface/deploy flow tests, `__tests__/reflectiveQuestionPipeline.test.ts`.
