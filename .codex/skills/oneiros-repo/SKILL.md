...
- **Reflective-question production hold:** recovered remote `ai-entitlements-gateway` v105 is `reflective-question-psychological-aliveness-v1.4.0` SHA `4885e351…`. Do not deploy local Oneiros Reader v1.4.0. Use `npm run deploy:ai-entitlements-gateway` only. Do not import `src/ai/reflectiveQuestionProductionHold.ts` into runtime. Guarded by flows-06 and `__tests__/flows/reflectiveQuestions.productionDeployGuard.contract.flow.test.ts`.
