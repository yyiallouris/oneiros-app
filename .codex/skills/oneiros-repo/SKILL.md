...
- **Reflective-question production:** recovered remote v105 method `reflective-question-psychological-aliveness-v1.4.0` SHA `4885e351…` is the git source of truth in `src/ai/reflectiveQuestionPrompt.ts`. Quick/chat: exactly 1 through the method. Standard/Advanced: 1–2 default 1. Essays stay `2.0.3-phase1` exactly one; do not inject the method into essay requests. R&D lives under `src/ai/rd/reflective-questions/` (Candidate B only) and must not be imported by runtime, nor `src/ai/reflectiveQuestionProductionHold.ts`. Deploy only through `npm run deploy:ai-entitlements-gateway`. Denied Oneiros Reader v1.4.0 SHA `0ea4b9a2…` cannot be env-approved. Guarded by `__tests__/flows/reflectiveQuestions.productionSurfaces.contract.flow.test.ts` and `__tests__/flows/reflectiveQuestions.productionDeployGuard.contract.flow.test.ts`.
