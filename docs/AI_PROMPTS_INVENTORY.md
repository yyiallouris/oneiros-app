...
| Reflective-question production hold | recovered remote `ai-entitlements-gateway` v105 is `reflective-question-psychological-aliveness-v1.4.0` SHA `4885e351ff6a20ceb8257c004aacd66e390e4c0902455a1cf5ff4d0df5a0238d`; local Oneiros Reader v1.4.0 SHA `0ea4b9a2…` is archived `DO NOT DEPLOY`; fail-closed wrapper `npm run deploy:ai-entitlements-gateway`; R&D isolated at `src/ai/rd/reflective-questions/` with Candidate B SHA `08cd3eaf…` as the only active research base; snapshot in `src/ai/reflectiveQuestionProductionHold.ts` is not imported by client/gateway runtime; [`REFLECTIVE_QUESTION_PRODUCTION_HOLD.md`](./REFLECTIVE_QUESTION_PRODUCTION_HOLD.md) |
