...
Extractions persist `dream-field-map-interpretive-v3.6` / schema `4` / `prompt_version` `3.6.3`. Path: dream + reflection → one `dream_extraction` call → Fabric + Archetypal Echoes + Mythic Echo or `[]` → lightweight validators → persist/UI. No production archetype `evaluation` bag. Mythic titles must name a narrative/episode (not a bare figure). Precision still prefers `[]` over a false echo, but highly distinctive multi-stage structural matches should be returned. Optional `debug_interpretive_echoes` returns diagnostics (never DB/UI).
