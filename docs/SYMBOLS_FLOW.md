...
2. The Reader writes questions in the same markdown as the reading. Streaming reveals that reading, including questions. Follow-up chat continues the conversation. Approved identity: `oneiros-same-call-reflective-questions-v1.0.0`. This changes no extraction input, Echo selection, `display_distillation`, `archetypes`, `amplifications`, or catalogs.
