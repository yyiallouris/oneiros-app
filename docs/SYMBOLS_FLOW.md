...
2. After the frozen Reader + v1.2 Generator finishes, production orchestration always writes exactly one schema-11 question (Integrity Gate, Premise Check, at most one Repair, then localized fallback). Streaming still reveals only reading bytes. Chat remains Dialogue `1.9.1` plus optional v5 questions. Approved identity: `oneiros-reflective-question-production-v1.0.0`. This changes no extraction input, Echo selection, `display_distillation`, `archetypes`, `amplifications`, or catalogs.
