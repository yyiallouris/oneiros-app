...
> **2026-08-27 reflective-question production hold:** recovered remote `ai-entitlements-gateway` v105 is `reflective-question-psychological-aliveness-v1.4.0` SHA `4885e351…`, not a verified `v1.5.0` method id. R&D is isolated under `src/ai/rd/reflective-questions/`. Deploy only through `npm run deploy:ai-entitlements-gateway`. Record: [`REFLECTIVE_QUESTION_PRODUCTION_HOLD.md`](./REFLECTIVE_QUESTION_PRODUCTION_HOLD.md). No Echo prompt, extraction, catalog, or runtime question prompt changed.
