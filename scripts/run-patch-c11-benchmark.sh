#!/usr/bin/env bash
# Patch C.1 targeted benchmark — no five-dream suite until acceptance passes.
set -euo pipefail
cd "$(dirname "$0")/.."
...
if not m: raise SystemExit("no service_role from supabase cli")
print(m.group(1))
')

EMAIL="oneiros.patchc11+$(date +%s)@example.com"
PASS="PatchC1Test!$(openssl rand -hex 4)"
...
npx --yes tsx scripts/run-patch-c11-benchmark.ts "$@"
