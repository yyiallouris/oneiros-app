/**
 * Patch C.1 targeted benchmark — no tuning between runs.
 * Order: Inanna×7 → competitor contrast → Fisherman/Sisyphus/Orpheus×3 only.
 *
 * Run: bash scripts/run-patch-c1-benchmark.sh
 */
...
async function main() {
  if (DREAM_EXTRACTION_PROMPT_VERSION !== '4.1.5-C.1') {
    throw new Error(`Expected 4.1.5-C.1, got ${DREAM_EXTRACTION_PROMPT_VERSION}`);
  }
  const jobs = buildJobs();
  const stamp = new Date().toISOString().replace(/[:.]/g, '-');
  const outDir = path.join(process.cwd(), 'tmp', `patch-c1-benchmark-${stamp}`);
