#!/usr/bin/env bash
# Patch F DIAGNOSTIC-ONLY — archetype selection stability (no production changes).
set -euo pipefail
cd "$(dirname "$0")/.."
...
EMAIL="oneiros.patch.f.stability+$(date +%s)@example.com"
PASS="PatchFStab!$(openssl rand -hex 4)"
...
# Max concurrency to save wall time; 429 exponential backoff lives in extractGlobalArchetypeFixture.
export PATCH_F_CONCURRENCY="${PATCH_F_CONCURRENCY:-8}"

npx --yes tsx scripts/run-patch-f-stability.ts
