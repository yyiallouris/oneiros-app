#!/usr/bin/env bash
# Patch C live benchmark — sequential, no tuning between runs.
# Usage: bash scripts/run-patch-c-benchmark.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
EMAIL="oneiros.patchc+$(date +%s)@example.com"
PASS="PatchCTest!$(openssl rand -hex 4)"
...
npx --yes tsx scripts/run-patch-c-benchmark.ts "$@"
