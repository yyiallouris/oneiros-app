/**
 * One authorized frozen v1.0.3 Reader evaluation.
 *
...
const FIXTURE_PATH = path.join(
  process.cwd(),
  'testing/reflective-questions/v1.0.3-enacted-relation-evaluation-2026-08-29.json'
);
const FIXTURE_SHA256 =
  'cc60ad8ebe6e66a5982a6b20ef995e1c7687a51d9becdb0e2e9165d3672da1ce';
const BASELINE_PATH = path.join(
  process.cwd(),
  'testing/reflective-questions/artifacts/v1.0.1-production-diagnostic-2026-08-29/RAW_RESULTS.json'
);
const BASELINE_FIXTURE_SHA256 =
...
const APPROVAL_ENV = 'ONEIROS_V103_ENACTED_RELATION_COST_APPROVED';
const HARD_COST_CAP_USD = 1;
const RESERVED_CALL_COST_USD = 0.045;
const EXPECTED_CALLS = 21;
const REQUEST_TIMEOUT_MS = 60_000;

type Cohort = 'known_failure_anchor' | 'strong_control' | 'sealed_unseen_holdout';
type BaselineGeneration = {
  generation_id: string;
  case_id: string;
  title: string;
  dream: string;
  mode: V103EnactedRelationDepth;
  language: string;
  surface: 'reading_standard' | 'reading_advanced';
  output: string;
...
type Fixture = {
  ...
  human_q1_pass_criteria: string[];
  structural_pass_criteria: string[];
  known_failure_anchors: string[];
  strong_controls: string[];
...
async function proxyCall(params: {
  ...
  anonKey: string;
  token: string;
  request: ReturnType<typeof buildV103EnactedRelationInitialRequest>;
}): Promise<ProxyResult> {
  ...
  try {
    response = await fetch(params.endpoint, {
      ...
      body: JSON.stringify({
        // Exact production gateway payload hint; task-config owns actual routing.
        model: 'gpt-5.4-mini',
...
function markdownReviewPacket(params: {
  ...
  return [
    '# v1.0.3 enacted-relation frozen human review packet',
    '',
    ...
    '',
    'Review Q1 against the seven frozen human criteria. Validator output is separate evidence and never determines the human verdict.',
    '',
    ...params.results.flatMap((result) => {
      ...
      return [
        ...
        '~~~',
        '',
        ...(before?.output ? ['### Before v1.0.1', '~~~text', before.output, '~~~', ''] : []),
        '### v1.0.3 candidate',
        '~~~text',
        after?.output ?? `[operational error: ${String(result.operational_error ?? 'unknown')}]`,
        '~~~',
        '',
        'Human Q1 verdict: PENDING',
...
async function main(): Promise<void> {
  ...
  if (baseline.fixture_sha256 !== BASELINE_FIXTURE_SHA256) {
    throw new Error('Production baseline fixture drifted.');
  }
  if (
    hashReflectiveQuestionPrompt(V103_ENACTED_RELATION_BUNDLE) !==
      V103_ENACTED_RELATION_BUNDLE_SHA256 ||
    fixture.candidate.method_id !== V103_ENACTED_RELATION_METHOD_ID ||
    fixture.candidate.bundle_sha256 !== V103_ENACTED_RELATION_BUNDLE_SHA256
  ) {
    throw new Error('Frozen v1.0.3 candidate identity drifted.');
  }
  if (
    ...
    throw new Error('Production v1.0.1 identity drifted.');
  }

  const priorEntries = [
    ...fixture.known_failure_anchors.map((generationId) => ({
      generationId,
      cohort: 'known_failure_anchor' as const,
    })),
    ...fixture.strong_controls.map((generationId) => ({
      generationId,
      cohort: 'strong_control' as const,
    })),
  ].map(({ generationId, cohort }): EvaluationEntry => {
    const generation = baseline.generations.find((item) => item.generation_id === generationId);
    if (!generation) throw new Error(`Missing frozen baseline case ${generationId}.`);
    ...
    return {
      ...
      before: {
        ...
        questions: generation.extracted_reflective_questions,
        validation: generation.contract_validation,
      },
    };
  });
  ...
  const preflight = {
    ...
    candidate_method: V103_ENACTED_RELATION_METHOD_ID,
    candidate_sha256: V103_ENACTED_RELATION_BUNDLE_SHA256,
    ...
    known_failure_anchors: fixture.known_failure_anchors.length,
    strong_controls: fixture.strong_controls.length,
  ...
  for (const entry of entries) {
    ...
    const request = buildV103EnactedRelationInitialRequest(dream, entry.mode);
    try {
      ...
      const output = call.content.replace(END_MARKER_DREAM_READING, '').trim();
      ...
      results.push({
        ...
        human_q1_verdict: 'PENDING',
        control_equivalence: entry.cohort === 'strong_control' ? 'PENDING' : null,
        failure_families: [],
        human_notes: 'PENDING',
      });
      ...
      results.push({
        ...
        estimated_usd: null,
        human_q1_verdict: 'NOT_RUN_OPERATIONAL_ERROR',
  ...
  writeFileSync(REVIEW_PACKET_PATH, markdownReviewPacket({
    candidateMethod: V103_ENACTED_RELATION_METHOD_ID,
    candidateSha: V103_ENACTED_RELATION_BUNDLE_SHA256,
