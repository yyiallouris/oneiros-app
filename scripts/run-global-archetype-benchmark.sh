#!/usr/bin/env bash
# Global archetype evaluation benchmark (59 frozen fixtures, 1 run each).
# No production changes — observability / confusion-matrix phase only.
set -euo pipefail
cd "$(dirname "$0")/.."
...
EMAIL="oneiros.global.archetype+$(date +%s)@example.com"
PASS="GlobalArchetype!$(openssl rand -hex 4)"
...
export GLOBAL_BENCHMARK_CONCURRENCY="${GLOBAL_BENCHMARK_CONCURRENCY:-2}"

npx --yes tsx scripts/run-global-archetype-benchmark.ts
