#!/usr/bin/env bash
# Live 5-dream combination acceptance suite (closed Mythic catalog).
# Dreams/expectations: docs/ONEIROS_5_DREAM_ACCEPTANCE_SET.{md,jsonl}
#
# Usage:
#   bash scripts/run-5-dream-acceptance.sh
#   ACCEPTANCE_CONCURRENCY=8 bash scripts/run-5-dream-acceptance.sh
set -euo pipefail
cd "$(dirname "$0")/.."
...
EMAIL="oneiros.5dream+$(date +%s)@example.com"
PASS="FiveDreamTest!$(openssl rand -hex 4)"
...
# Default: fully parallel (all jobs). Set ACCEPTANCE_CONCURRENCY only to throttle.
export ACCEPTANCE_RUNS_PER_DREAM="${ACCEPTANCE_RUNS_PER_DREAM:-3}"
# Leave ACCEPTANCE_CONCURRENCY unset → runner uses job count.
if [ -n "${ACCEPTANCE_CONCURRENCY:-}" ]; then
  export ACCEPTANCE_CONCURRENCY
fi

npx --yes tsx scripts/run-5-dream-acceptance.ts
